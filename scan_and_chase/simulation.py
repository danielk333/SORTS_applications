...
class ScanAndChaseSim(sorts.Simulation):
    ...
    def plot(self, dat):
        ...
        max_snr = 10 * np.log10(
            passes[0].calculate_snr(radar.tx[0], radar.rx[0], obj.d)
        )
        ...
        axes[1][1].plot(
            data[0]["t"] / 60.0,
            10 * np.log10(data[0]["snr"]),
            "xk",
            alpha=1,
            label="Scan",
        )

        _, axes = sorts.plotting.observed_parameters(
            [scan_and_chase_datas[0]], passes=[passes[0]], axes=axes
        )
    ...
    def iterative_det_od(self, index, **kwargs):
        ...
        t, states, passes, data = detect.get_detections(
            scheduler, obj, self.logger, profiler, t_samp=10.0
        )
